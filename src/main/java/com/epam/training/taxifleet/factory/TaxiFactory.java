...
public class TaxiFactory {
	...
	public InternalCombustionEngine getPetrolEngine(int numberOfCylinders, int displacement, 
			int maxPower, int maxTorque, boolean turbocharged) throws IllegalSetValueException {
	...
	public InternalCombustionEngine getDieselEngine(int numberOfCylinders, int displacement,
			int maxPower, int maxTorque, boolean turbocharged) throws IllegalSetValueException {
	...
	public PassengerCar getCombustionEnginePassengerCar(int carID, int carPrice, int topSpeed,
			int curbWeight, double consumption, BodyStyle bodyStyle,
			DriveArrangement driveArrangement, Gearbox gearbox, 
			InternalCombustionEngine carEngine) throws IllegalSetValueException {
		PassengerCar passengerCar = new CombustionEnginePassengerCar(gearbox, carEngine);
			
		initializePassengerCar(carID, carPrice, topSpeed, curbWeight, consumption, 
					bodyStyle, driveArrangement, passengerCar);
		return passengerCar;
	}
	
	// get an electric car entity
	public PassengerCar getElectricPassengerCar(int carID, int carPrice, int topSpeed, int curbWeight,
			double consumption, BodyStyle bodyStyle, DriveArrangement driveArrangement, 
	...
	public PassengerCar getHybridPassengerCar(int carID, int carPrice, int topSpeed, int curbWeight,
			double consumption, BodyStyle bodyStyle, DriveArrangement driveArrangement,
			InternalCombustionEngine carMainEngine,	ElectricEngine carAdditionalEngine) 
					throws IllegalSetValueException{
		PassengerCar passengerCar = new HybridPassengerCar(carMainEngine, carAdditionalEngine);
	...
	private boolean initializeEngine(int numberOfCylinders, 
	...
	// supplementary method
	private boolean initializePassengerCar(int carID, int carPrice, int topSpeed,
			int curbWeight, double consumption, BodyStyle bodyStyle,
			DriveArrangement driveArrangement, PassengerCar passengerCar) throws IllegalSetValueException {
		boolean isInitialized = false;
